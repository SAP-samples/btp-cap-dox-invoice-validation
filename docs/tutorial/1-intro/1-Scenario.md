...
2. The invoice PDF will be preprocessed by **SAP Document Information Extraction (DOX)** Service.
...
As illustrated above, the application's core purpose is to validate invoices received via e-mail or physical mail (scanned into PDF format). These invoices are stored via the **BTP Object Store Service** in an arbitrary object store, e.g. **Amazon S3 Object Store**, ensuring that a persistent version of each invoice is available.
...
In this case, **SAP Document Information Extraction (DOX)** Service is used to extract values of invoices in a structured way. Invoices that are being corrected with this application, contain the following information per invoice entry:
